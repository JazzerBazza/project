...
def about():
    return render_template(
        ...
        names=['Barrie', 'Rob', 'Sean']
    )


@app.route('/add_to_cart/<int:product_id>/<int:customer_id>/', methods=['GET', 'POST'])
def add_to_cart(product_id, customer_id):
    cursor = get_db().cursor()
    cursor.execute("INSERT INTO cart (Product_id, Customer_id) VALUES (%s, %s)", (product_id, customer_id))
    if session:
