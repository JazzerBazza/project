...
def connect_db():
    return pymysql.connect(
        user = 'root', password = 'password', database = 'Cohort4v2',
...
@app.route('/add_to_cart/<int:product_id>/product_name/<int:customer_id>/', methods=['GET', 'POST'])
def add_to_cart(product_id, product_name, customer_id):
    cursor = get_db().cursor()
    cursor.execute("INSERT INTO cart (Product_id, Product_name, Customer_id) VALUES (%s, %s, %s)", (product_id, product_name, customer_id))
    if session:
...
def cart():
    cursor = get_db().cursor()
    cursor.execute("SELECT Product_id, Product_name, Customer_id from cart")
    result = cursor.fetchall()
    app.logger.info(result)
    return render_template(
                'cart.html',
...
def empty_cart():
    cursor = get_db().cursor()
    cursor.execute("SELECT Product_id, Customer_id from cart")
