...
def cart():
    cursor = get_db().cursor()
    cursor.execute("SELECT p.Product_id, p.Product_name, p.Price from cart c join products p where c.Product_id = p.Product_id")
    result = cursor.fetchall()
    app.logger.info(result)
    return render_template(
                'cart.html',
...
def empty_cart():
    cursor = get_db().cursor()
    cursor.execute("SELECT Product_id from cart")
